...
-- Keep the built-in chat types separate from numbered/custom channels such as "World".
-- IMPORTANT: do not append dynamic channels directly to DPSMate.L["gchannel"] or
-- DPSMate.L["reportchannel"], because those localization tables are also used to
-- decide whether SendChatMessage should receive RAID/PARTY/etc. as its chat type.
local function DPSMate_CopyReportChannelList(source)
	local result = {}
	if source then
...
local DPSMate_BuiltinReportChatTypes = {
	["Raid"] = "RAID",
	["Party"] = "PARTY",
	["Say"] = "SAY",
	["Officer"] = "OFFICER",
...
local function DPSMate_ResolveReportChannel(channel, whisperTarget)
	if channel == DPSMate.L["whisper"] then
		return "WHISPER", whisperTarget
	end

	local builtin = DPSMate_BuiltinReportChatTypes[channel]
	if builtin then
		return builtin, nil
	end

	-- Numbered/custom channels (World, LookingForGroup, etc.) must always use
	-- chat type CHANNEL with the numeric channel id as destination.
	local id = GetChannelName(channel)
	if type(id) == "number" and id > 0 then
		return "CHANNEL", id
	end

	-- Some 1.12/private-server clients are inconsistent when GetChannelName is
	-- called by name, so fall back to resolving the exact name from the channel list.
	for i = 1, 25 do
...
function DPSMate.Options:ChannelDropDown()
	local channel, i = DPSMate_CopyReportChannelList(DPSMate.L["reportchannel"]), 1
...
function DPSMate.Options:Report()
	...
	chn, index = DPSMate_ResolveReportChannel(channel, DPSMate_Report_Editbox:GetText())
	if not chn then
		DEFAULT_CHAT_FRAME:AddMessage("DPSMate: Could not resolve report channel '"..tostring(channel).."'.")
		return
	end
...
function DPSMate.Options:ReportUserDetails(obj, channel, name)
	...
	local chn, index = DPSMate_ResolveReportChannel(channel, name)
	if not chn then
		DEFAULT_CHAT_FRAME:AddMessage("DPSMate: Could not resolve report channel '"..tostring(channel).."'.")
		return
	end
...
function DPSMate.Options:InializePlayerDewDrop(obj)
	local channel, i = DPSMate_CopyReportChannelList(DPSMate.L["gchannel"]), 1
